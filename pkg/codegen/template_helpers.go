...
func genParamArgs(params []ParameterDefinition) string {
	...
	for i, p := range params {
		...
		parts[i] = fmt.Sprintf("%s %s", paramName, p.Prefix()+p.TypeDef())
	}
...
func genResponseUnmarshal(op *OperationDefinition) string {
	...
	// See: https://github.com/deviesdevelopment/oapi-codegen/issues/127 for why we handle this in two separate
	// groups.
